...
请按照 name salary rank(薪水排名)  按照薪水从高到低排序,实现下面的输出
	...
	select t.student_id, t.score,(select count(s.score)+1 from mock_enroll s where link_id=4   and s.score>t.score) rank from mock_enroll t where t.link_id=4  	   and student_id=22599 order by t.score desc;
